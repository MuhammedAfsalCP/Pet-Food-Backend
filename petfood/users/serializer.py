...
class UserRegisterSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if not re.search(r"[A-Za-z]", password):
            raise serializers.ValidationError({"password": "Password must contain at least one alphabetic character."})

        if not re.search(r"[0-9]", password):
            raise serializers.ValidationError({"password": "Password must contain at least one numeric character."})
