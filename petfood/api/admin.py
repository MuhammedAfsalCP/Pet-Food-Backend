...
class CustomUserAdmin(UserAdmin):
    model = User
    list_display = ('username', 'email', 'first_name', 'last_name', 'is_deleted', 'is_staff', 'is_active')  # Fields to display
    list_filter = ('is_deleted', 'is_staff', 'is_active')  # Filters on the right side of the admin page
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('username','email', 'password1', 'password2', 'is_deleted', 'is_active', 'is_staff', 'is_superuser')}
        ),
    )
